...
const ingestData = async (dataToIngest) => {
  try {
    ...
    if (response.status >= 200 && response.status < 300) {
      console.log("Data ingested successfully into the schema");
      return {
        success: true,
        message: "Data ingested successfully into the schema",
      };
    } else {
      console.error("Failed to ingest data into the schema", response.status);
      return {
        success: false,
        message: "Failed to ingest data into the schema",
...
app.post("/mib/webhook", async (req, res) => {
  ...
  // Call the separate ingestion function
  const ingestionResult = await ingestData(dataToIngest);

  if (ingestionResult.success) {
    res.status(200).json({ message: ingestionResult.message });
  } else {
    res
      .status(ingestionResult.status || 500)
      .json({ error: ingestionResult.message });
  }
});
...
//         .status(response.status)
//         .json({
//           error: errorData.message || "Failed to ingest data into the schema",
//         });
//     }
//   } catch (error) {
